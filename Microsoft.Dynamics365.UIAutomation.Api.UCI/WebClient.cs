    ...
    public class WebClient : BrowserPage
    {
        ...
        private LoginResult Login(IWebDriver driver, Uri uri, SecureString username, SecureString password, SecureString mfaSecretKey = null, Action<LoginRedirectEventArgs> redirectAction = null)
        {
            ...
            if (!waitingForOtc)
            {
                ...
                ThinkTime(5000);
                EnterOktaID(driver, "EYCRMCMTestUser10");
                ThinkTime(1000);
                EnterPassword(driver, password);
                ThinkTime(1000);
                Submit(driver);
                ThinkTime(2000);
                SecAnswer(driver, "sachin");
                ThinkTime(1000);
            }
        ...
        {
            var input = driver.WaitUntilAvailable(By.Name("loginfmt"), new TimeSpan(0, 0, 30));
            if (input == null)
                return false;
        ...
        {
            var input = driver.FindElement(By.Name("password"));
            input.SendKeys(password.ToUnsecureString());
            input.Submit();
        }

        private static void EnterOktaID(IWebDriver driver, String UserID)
        {
        ...
        private static void Submit(IWebDriver driver)
        {
            var submit = driver.FindElement(By.Id("okta-signin-submit"));
            submit.Click();
        }
        private static void SecAnswer(IWebDriver driver, String answer)
        {
            var input = driver.FindElement(By.Name("answer"));
...

        {
            ...
            driver.FindElement(By.Name("loginfmt")).SendKeys(args.Username.ToUnsecureString());
            driver.ClickWhenAvailable(By.Id("idSIButton9"), new TimeSpan(0, 0, 2));
            driver.FindElement(By.Name("username")).SendKeys("EYCRMCMTestUser10");
            driver.FindElement(By.Name("password")).SendKeys(args.Password.ToUnsecureString());
            driver.ClickWhenAvailable(By.Id("okta-signin-submit"), new TimeSpan(0, 0, 2));
            driver.FindElement(By.Name("answer")).SendKeys("sachin");
            driver.ClickWhenAvailable(By.XPath("//input[@value='Verify']"), new TimeSpan(0, 0, 2));
